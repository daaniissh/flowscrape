      ...
      <div className="flex flex-col p-2 ">
        {routes.map(route => (
          <Link key={route.href} href={route.href} className={buttonVariants({
            variant: activeRoute.href === route.href ? "sideBarActivateVariant" : "sideBarItem"
          })} > <route.icon size={20} />{route.label} </Link>
        ))}
      </div>
    </div>
  )
  ...
  return <div className='block border-separate bg-background md:hidden' >
    <nav className="container flex-center">
      <Sheet open={isOpen} onOpenChange={setOpen}>
        ...
        <SheetContent className='w-[400px]  sm:w-[540px] space-y-4 p-6 ' side='left' >
          <Logo />
          <div className="flex flex-col gap-1">
            {routes.map(route => (
              <Link onClick={(prev)=>setOpen(!prev)} key={route.href} href={route.href} className={buttonVariants({
                variant: activeRoute.href === route.href ? "sideBarActivateVariant" : "sideBarItem"
              })} > <route.icon size={20} />{route.label} </Link>
            ))}
          </div>
